    ...
    for n in range(len(universe)-1):
        ...
        for rows in range(6240):
            for fields in range(5120):            
                if (int(after[rows][fields])!=0) and (int(before[rows][fields])== 0):
                    PopulationDifferent = int(after[rows][fields])
...
def drawpopulationmap(matx,year):
    ...
    im.save("data/outputdata/step3_emergencepop/emergencepop" + str(year) + '.tif')
    print("Map is output")

...
if __name__ == "__main__":
    ...
    with open("data/step5_global_agglomeration_data/global_agglomerations_final.csv" , "r", encoding="utf-8") as input_file:
    ...
    for (index,EachGlobalAgglo) in enumerate(GlobalAggloIDFullLIST):
        ...
        for EachRegionalAgglo in EachGlobalAgglo:            
            ...
            temp_list = []
            with open("data/step2_regional_agglomeration_data/agglomeration_sorted_"+str(meshID)+".csv" , "r" , encoding="utf-8") as input_file2:
            ...
            TotalCell += int(temp_list[AggloID][1])
        
        GlobalAggloIDFullLIST[index].insert(0,TotalCell)
        
    GlobalAggloIDFullLIST.sort(reverse=True)
    
    filename = "data/step5_global_agglomeration_data/global_agglomerations_final_sorted.csv"
    metadata.writeout(GlobalAggloIDFullLIST,filename)
    
    #with open("data/step5_global_agglomeration_data/global_agglomerations_final_sorted.csv" , "w" , encoding="utf-8") as output_file:
    #    for each_row in GlobalAggloIDFullLIST:
    #        for each_field in each_row:
    #            output_file.write(str(each_field)+",")
    #        output_file.write("\n")

    print("#####################\nPart 5.5 program END. Global Agglomerations Fulllist is output.\n#####################")

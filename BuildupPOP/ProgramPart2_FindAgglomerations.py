...
if __name__ == "__main__":
    ...
    #while True:
        ...
        #    break
        #else :
        DistributionMap = metadata.call_landuse_distribution_map(meshID)
        ...
        for rows in range(800):
            for fields in range(800):
                ...
                if str(DistributionMap[rows][fields]) == "0700":
        ...
        for rows in RegionalAggloLIST :
            ...
            for each_field in rows:
                (Y,X) = each_field 
                CellID = str(meshID)+"_"+str(Y)+"_"+str(X)
                templist.append(CellID)
            Final_RegionalAggloLIST.append(templist)
                
        
        filename = "data/step2_regional_agglomeration_data/agglomeration_" + str(meshID) + ".csv"

    ...
    #while True:
        ...
        for PopCoorY in range(160):
            ...
            for PopCoorX in range(160):
                ...
                populationstddev.append(deviation)
                
    populationmean = sum(populationstddev)/len(populationstddev)

    varient = []
    for x in populationstddev:
        buffer = x - populationmean
        buffer = buffer*buffer
        varient.append(buffer)

    sumofdx = sum(varient)
    stddev = (sumofdx/len(varient))**(1/2)
    print("mean=",populationmean)
    print("min=",min(populationstddev))
    print("max=",max(populationstddev))
    print("stddev=",stddev)

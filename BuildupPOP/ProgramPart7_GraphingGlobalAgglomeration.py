...
def gen_scale():
    ...
    im.save("data/step7_graphing_global_agglomeration/scale.tif")      
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if query != int:
            ...
            bigpicture = []
            with open("data/step6_bigpicture/bigpicture.csv" , "r" , encoding="utf-8") as input_file:
            ...
            print("Global Agglomeration you selected is located in mesh\n",
                    ...
                    "with total cells of", int(QuestedGlobalAggloPopulation),".")
            ...
            for eachMeshDATAs in MeshIDs:
                ...
                with open("data/step2_regional_agglomeration_data/agglomeration_sorted_"+str(meshID)+".csv" , "r" , encoding="utf-8") as input_file:
                    for line in input_file:
                        NewLine = line.strip()
                        #NewLine = NewLine[:-1]
                        NewLine = NewLine.strip().split(",")
                ...
                for eachAggloDATA in NeededDATA:
                    ...
                    for eachCell in cellDATAs:
                        ...
                        cellPopulation = 1 #int(CellINFO[3])    
            ...
            #filename = "data/step7_graphing_global_agglomeration/map1/" + str(filename) + ".csv"
            #metadata.writeout(trimedmatrix,filename)          
            #with open("data/step7_graphing_global_agglomeration/map1/"+str(filename)+".csv" , "w" , encoding="utf-8") as output_file:
            ...
            #g = open("data/step7_graphing_global_agglomeration/map1/" + str(filename) + ".csv" , "r" , encoding="utf-8")
            ...
            for x in range(width):
                for y in range(height):
                    ...
                    if temp[y,x] == 0:
                        ...
                        #pix[x,y] = (int(round(255 - (255)/(1+6.96875*2.718281828**(-(0.0030937483*(temp[y,x])))))),
                        #            int(round(255 - (255)/(1+6.96875*2.718281828**(-(0.0030937483*(temp[y,x])))))),
                        #            int(round(255 - (255)/(1+6.96875*2.718281828**(-(0.0030937483*(temp[y,x])))))))
                    # (R,G,B)
                    #要用顏色深淺黎表達人口密度
                    #人口密度上限係10000
            ...
            im.save("data/step7_graphing_global_agglomeration/map1/"+str(filename) + '.tif')
